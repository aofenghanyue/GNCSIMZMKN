...
- `legacy_simflow_reference.py`：核对冻结 Legacy SimFlow 来源、环境、运行测试、capture harness、两份 effective mission 和四份 dataset，独立完成按字段 identity 的 matrix row 物化，并与 C++17 typed probe 交叉验证 requested-input 声明与 case-source 列重排、普通 `--config` 重放、独立全新工作目录、case 目录外输入和五条直接失败路径。
