...
- `validate-license-provenance.ps1`：核对全部 Git 跟踪文件的分发范围、需审查 binary/archive、实际 CMake/CI 外部输入和 Legacy archive 身份；默认确认内部工作区事实并报告外部分发 blockers，`-RequireExternalReady` 可直接拒绝未闭合的外发候选。
