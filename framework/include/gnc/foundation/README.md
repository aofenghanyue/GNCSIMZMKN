...
`PreparedTrilinearTableView` 不拥有轴或表格内存；调用方需保证其只读存储覆盖准备结果及全部查询的生命周期。当前求根器只覆盖具有连续性和显式 bracket 的标量问题，尚未提供导数、初值式局部求解或多变量求根。当前矩阵求解只覆盖固定 `3×3` SPD 问题；动态矩阵分解、其他维度或策略的插值和自适应积分尚未进入当前切片。Foundation 四元数只承载数值存储和纯算法；frame/time 类型归属 R1 Contracts，Euler 转换仍要求调用方提供完整 profile metadata。
