...
`ref-legacy-phase/reference.json` is a capturing Legacy behavior slice. It verifies two raw traces from an external harness compiled against the frozen archive, compares them with an independent C++17 scheduler probe, and rejects phase swaps and duplicate phases. Its macro-order Preserve recommendation and Legacy ordering-surface Retire recommendation remain pending.
