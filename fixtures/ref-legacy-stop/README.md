...
The two record-field events may exchange relative order after mapping by `field_id`; both must remain between publish and termination evaluation. Their encoded dataset columns may exchange order after mapping by header identity.
...
The comparison independently rejects termination-before-record, a row that is unavailable when evaluation returns, a missing terminal row, an advanced final time and an observation after the stopping boundary. Swapping record-field events and their dataset columns, along with changing Legacy free-text reason, preserves semantic equivalence. The accepted disposition preserves a terminal `Observation` for the stopping state before its `RunOutcome` while retiring Legacy reason text, ordering rules and runtime/CSV surfaces. The bundle is `executable`; target artifacts and typed termination identity remain pending future-stage implementation.
