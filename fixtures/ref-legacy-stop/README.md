...
The comparison independently rejects termination-before-record, a row that is unavailable when evaluation returns, a missing terminal row, an advanced final time and an observation after the stopping boundary. Swapping record-field events and their dataset columns, along with changing Legacy free-text reason, preserves semantic equivalence. Target `Observation` and `RunOutcome` artifacts remain pending future-stage implementation. The bundle stays `capturing` until the repository owner accepts the recommended Preserve/Retire split.
