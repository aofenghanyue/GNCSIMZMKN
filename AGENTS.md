...
本仓库是 GNCZMKN 目标架构的全新实现。当前处于 R0，产品代码仍为空骨架。蓝图描述目标语义，已交付能力只以当前源码、fixture、oracle 和自动测试为准。
...
- 当前 gate 为 R0。
- R0 允许科学约定、独立 reference、fixture/oracle、必要 schema 和关键架构边界检查。
- R1～R8 的生产能力保持锁定，直到仓库所有者明确改变范围或 R0 gate 通过。
