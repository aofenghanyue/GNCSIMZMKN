...
- 实现成熟度：`ORACLE-YYZ-PUBLISH-01`、`ORACLE-YYZ-PHASE-02` 与 `ORACLE-YYZ-SYNC-03` 已达到 `executable`；`ORACLE-YYZ-GROUP-04`、`ORACLE-YYZ-CSV-05`、`ORACLE-YYZ-STOP-06` 与 `ORACLE-SIMFLOW-07` 处于 `capturing`。七条均有可执行证据 bundle 和直接失败路径；待定处置仍阻断完整 G1 或 runtime migration pass evidence
...
- `R0-LEG-001` 已完成，`R0-LEG-002` 保持 `in_progress`；publish、phase 与同步提交切片的处置已接受，continuous-group、CSV、STOP 与 SimFlow 切片仍显式待定；
