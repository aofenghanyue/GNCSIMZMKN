...
| `ORACLE-SIMFLOW-07` | base mission + 两行 variation matrix + numeric perturbation materializer | `hot` source case 被物化；effective mission 含 `engine.temp_level=2` 与 `aero.drag_bias=-0.03`；普通 `gnc_sim --config` 可重放 | 已在两个独立新工作目录运行 `--simflow`，把 effective mission 复制到 case 目录外，再从另外两个独立全新目录运行普通 `--config`；两份 effective mission、两份 summary 和四份 dataset 分别 byte-identical，并由独立 Python 物化器与 C++17 typed replay probe 复核；effective-mission JSON 重编码、requested-input 声明、case-source 列、dataset 全列重排、target case-manifest presence 和 CLI spelling 变化保持同一语义结果 | source row/input identity、command input-role/order、exit 与 row identity exact；dataset scalar 1e-12；Legacy entrypoint/mode spelling、input 声明顺序、case-source 列序、目录、numeric index、case-manifest absence、JSON 格式与 dataset CSV 列序排除；target CaseId 与 manifest 内容/identity 待定 | 已接受：Preserve 预运行自包含任务物化与 ordinary compile/run replay（DeclaredModelChoice）；Retire `case_000001`、目录/index、case-manifest absence、Legacy API、CLI spelling 与 old Mission shape；deterministic target CaseId 为新契约 |
...
`ORACLE-YYZ-SYNC-03` 的直接失败用例会在 position candidate 完成前提交 mass；Python evaluator 和 C++ probe 都拒绝该 journal，并证明错误路径得到 `position=8`。`ORACLE-YYZ-PUBLISH-01` 的直接失败用例在 publish 内把 committed altitude 增加 `1 m`，C++ probe 拒绝该状态变化，Python comparator 同时核对失败事实。`ORACLE-YYZ-PHASE-02` 拒绝 process/output 交换和重复 input phase。`ORACLE-YYZ-GROUP-04` 拒绝得到 `position=10` 的 split snapshot closure、未注册 member 和重复 scope ownership。`ORACLE-YYZ-CSV-05` 接受列置换、有限等值数值文本、未映射列变化和重复未映射表头对应的语义等价 dataset，并拒绝缺失 t0、缺失必要列、错位 `t_k`、陈旧发布态、非有限必要字段值和重复必要表头。`ORACLE-YYZ-STOP-06` 分别拒绝提前终止、评估时终止行不可见、缺失终止行、终态时间推进和停止后额外观测。`ORACLE-SIMFLOW-07` 拒绝缺失注入输入、用批处理物化 input role 隐藏普通重放上下文、复用批处理工作目录、把普通重放输入留在 case 目录内，以及普通重放结果漂移；Legacy case 目录、numeric index、case-manifest presence 与 CLI spelling 变化不影响语义等价。重复 id、跨平台聚合 hash、通用 classification completeness 等检查留到出现当前 consumer 或直接回归后再增加。
...
- `R0-LEG-001` 与 `R0-LEG-002` 已完成，七个切片的处置均已接受并由可执行 bundle 闭合；
