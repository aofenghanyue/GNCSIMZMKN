...
- 自动反例覆盖重复 identity、悬空引用、循环依赖、禁止边和源哈希漂移；
...
- failure review：损坏表格、重复术语、悬空 alias、enum 值漂移、缺失权威、缺失 Legacy owner、依赖环、禁止 Kernel 边和 source-hash 漂移共 9 个反例均被拒绝；
...
- verification：4/4 CTest、repository verification、9 个自动反例和 `git diff --check` 通过；
