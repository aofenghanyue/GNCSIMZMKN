...
现有自动检查已经能证明术语/authority 唯一性、ADR-0003/CMake module DAG、Legacy ownership 与派生 hash 一致性，并有 9 个负向 mutation。仓库总验证另有少量 Legacy include/token 和 `kernel -> compiler` 检查。现有证据尚未覆盖 source include 方向、完整 Legacy 删除词、延期能力禁入、精确 exception 规则和未来 descriptor/state/transaction 语义。
...
- 上表至少 14 个 mutation 全部由生产 evaluator 拒绝；
...
- 当前证据审计：记录 architecture baseline 的 9 个 mutation、CMake DAG/authority 强覆盖和 repository regex 的 partial 覆盖；所有 product-artifact 语义保持 awaiting/deferred，没有借零文件数量声明通过。
- 故障设计：ARCH-MUT-001～014 覆盖 module/include、Plan/Commit/Application 防火墙、Legacy、延期能力、ADR graph、exception 和路径规范化；要求 mutation 与 positive scan 共用 evaluator。
