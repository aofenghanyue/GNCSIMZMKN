...
1. semantic byte encoding identity 固定为 `gnc.canonical-mission-ir.semantic-bytes@1`，hash algorithm 固定为 SHA-256。
...
3. collection 按 canonical key 唯一排序。hash 前重新验证 IR 的 canonical order、identity、scope/subject 关系、端口、配置值、asset 与 binding endpoint。非 canonical graph 在进入 SHA-256 前失败。
4. hash 覆盖 mission revision/id、entity、scope、model/package/preparation identity、execution form、placement、subject/scope relation、ports、canonical config、asset binding、algorithm consumer 与 binding intent。
...
- entity、scope relation、placement、model、config 或 asset 的真实语义变化会改变 digest。
