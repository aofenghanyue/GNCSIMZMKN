...
- `R1-MOD-001` 已完成。公共 metadata 收敛为 stable model id/version、`PureQuery | Closure` 和 preparation identity；CAVH GlideEnvelope 与 R2 提取的 YYZ AerodynamicTable 使用 PureQuery，YYZ ForceMomentClosure 使用 Closure。clock/configuration expectation 留在 package-specific definition，`RuntimeComponent` 已退出当前公共有效形式。
...
- canonical hash validator 现与 Source/IR builder 共用同一 composition-node namespace 规则，并在 SHA-256 前拒绝 model/algorithm 跨集合 identity 冲突、空 model output 和空 algorithm input。C++ 直接非法 IR 与独立 Python reference 具有等价负例；`semantic-bytes@2` vector 保持 `b29dc67f2a9e0bb36cb18a5e54a8c4830bdb0cae718fbf856646ba903892511b`。Catalog 同时拒绝非法 `CanonicalConfigValueKind` 枚举值。

## 后续阶段边界

1. `R2-CAT-001`、`R2-PLAN-001`、`R2-PRF-001`、`R2-LINK-001` 与 `R2-GATE-001` 保持 `planned`。首个真实 RuntimeComponent 必须同时带来独立 state ownership、调度、command/lifecycle 或 topology consumer；届时只冻结该 consumer 直接需要的 recipe/profile、ports/state、obligation、lifecycle、owner/authority/topology 字段与 sampled graph analysis。
2. 当前产品仅在 `ControlledPropelledRigidMassStepKernel` 内联执行 guidance、controller、ideal actuator、propulsion、rigid 与 mass。R0 fixture 的每 tick schedule 不能单独提升为产品 RuntimeComponent 调度；完整闭合图还缺这些相邻 runtime descriptors，以及 committed-observation、environment 和 run-binding provider。`AltitudePitchGuidanceOutput` 同时承载 controller 输入和解释性 telemetry，正式 runtime contract 还需要先分离。
3. `RigidStepKernel` 当前自行执行完整 RK4，缺少独立 derivative/projection entry、StateSchema 和 environment/mass/wrench provider。将其直接声明为 `ContinuousStateOwner`，或把完整 wrapper 声明为单一 owner，都会改变公共 ownership/time/transaction 语义并跨入 R3；这些选择需要仓库所有者先确定产品边界。
4. StateOwner、DecisionAuthority、entity activation/topology、intervention/fault routing 和 RuntimeComponent sampled graph cycle analysis 均未由当前 PureQuery/Closure 图触发。既有 rigid/mass atomic boundary 继续归入未来 `IntegrationScopePlan` 与 `StepTransaction` 的多 owner 原子协调。
5. JSON/YAML/INI、多端 adapter、Session、mini runtime、manager、runtime registry、serializer、StateFragment、Artifact、Workflow、前端与 R3+ 能力继续保持关闭。
