...
1. 继续 `R0-SCI-003` 单一主线，把 `REF-YYZ-SCALAR-BURN-MASS-001` 的 tick-1 committed 质量接入第二个 FrozenInterval，形成 candidate → commit → next-sample consumer 的两区间轨迹回归；继续保持 constant geometry 与 canonical fuel/vehicle assets 范围隔离。
