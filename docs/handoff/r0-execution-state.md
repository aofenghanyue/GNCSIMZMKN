...
- `R2-CAT-001` 已形成 owner-review 候选。YYZ package 把真实 `AltitudePitchGuidanceKernel` 描述为 stateless `SampledTransform + BoundaryEvaluation`：`vehicle.process`、process phase 每步、current-cycle sampled input/output、zero-order hold、空 state schema、instantiate/dispose 和 exact recipe/kernel identity。canonical config 可确定性重建 guidance definition；Catalog snapshot 与 duplicate/invalid form/profile/recipe/obligation/schedule/port/lifecycle 负例由 `r2.compiler-runtime-component-catalog.probe` 执行。`ADR-0015` 处于 Proposed，等待仓库所有者判断。
