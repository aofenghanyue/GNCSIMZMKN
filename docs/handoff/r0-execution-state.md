...
1. 选择 `R0-ARCH-002` 作为下一条 R0 主线，窄复核现有 architecture baseline 与 repository verification，只补会影响构建、运行、依赖方向、mutable-state owner 或 Legacy 隔离的正反例 guard。
2. 产品 schema、loader、Compiler、Session、durable evidence、gate 与 R1～R8 runtime 能力继续保持锁定；报告措辞、actor、commit/hash 和验收回执不进入 guard。
