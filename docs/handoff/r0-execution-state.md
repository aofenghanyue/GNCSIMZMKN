...
1. `R2-CAT-001` 为 `review`；`R2-PLAN-001`、`R2-PRF-001`、`R2-LINK-001` 与 `R2-GATE-001` 保持 `planned`。Compiler 解析 RuntimeComponent definition 后会以 `GNC-PLAN-RUNTIME-COMPONENT-UNAVAILABLE` 拒绝发布局部 IR/plan。
2. 当前最小阻碍是缺少可由 Compiler 连接的 committed-observation provider 与 environment/run-binding provider。产品 wrapper 内联 guidance/controller/actuator/propulsion/rigid/mass；把其中单个 kernel 独立降级会留下 required input，把 wrapper 视为单一 owner 会提前改变 R3 transaction ownership。
3. StateOwner、DecisionAuthority、entity activation/topology、intervention/fault routing 和 RuntimeComponent sampled graph cycle analysis 尚未被闭合 plan 触发。既有 rigid/mass atomic boundary 继续归入未来 `IntegrationScopePlan` 与 `StepTransaction` 的多 owner 原子协调。
