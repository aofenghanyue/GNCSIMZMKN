...
- `R0-SCI-003` 已进入 `in_progress`；仓库所有者已接受 fixture-local 刚体核心、四元数归一化策略与 `FrozenInterval` 力/矩闭合范围。`REF-YYZ-6DOF-CORE-001` 提供独立公式 intermediates、解析与高精度轨迹、收敛、转动守恒量、ExactGrid 终止及关键失败用例；`REF-YYZ-FORCE-MOMENT-CLOSURE-001` 提供逐来源力矩搬移、规范化闭合、重力分离、闭合到刚体核心的解析短轨迹、输入域拒绝与物理 mutation。完整 `REF-YYZ-001` 仍待 mission、环境、气动、推进、制导控制、终止指标与生产容差闭合。
- 其余 R0 任务保持 `planned`。

## 下一条开发主线

1. 继续 `R0-SCI-003` 单一主线，在已验证刚体核心与 `FrozenInterval` 闭合之后窄复核 supplied air-data kinematics 的公式、输入域和奇点策略。
