...
- `R2-CAT-001` 已形成 owner-review 候选。YYZ package 把真实 `AltitudePitchGuidanceKernel` 描述为 stateless `SampledTransform + BoundaryEvaluation`：`vehicle.process`、process phase 每个 committed boundary、current-cycle sampled input/output、zero-order hold、instantiate/dispose 和 exact recipe/kernel identity；该窄 descriptor 不开放 state schema。canonical config 可确定性重建 guidance definition；Catalog snapshot 与 duplicate/invalid form/profile/recipe/obligation/schedule/port/lifecycle 负例由 `r2.compiler-runtime-component-catalog.probe` 执行。`ADR-0015` 处于 Proposed，等待仓库所有者判断。
- R2-PLAN 的不受阻子切片已把 Descriptor 升为 revision 3：全部三个现有 occurrence 形成 `PreparedModelPreparationInputs`，保留 exact prepare identity、完整 canonical config、asset-binding refs 和稳定 source refs；CAVH envelope/YYZ aero 形成 `QueryExecutionSpecInputs`，YYZ closure 形成带 `FrozenInterval` 的 `ClosureExecutionSpecInputs`。每项 execution input 都显式声明无 workspace；consumer-binding facts 从已解析 BindingPlan 派生并保存 provider/consumer/contract/scope/temporal cross-reference。它们只复述结果流，不授予 BoundQuery/Closure handle 调用权；obligation 保存稳定 execution-input ref。`ClosureStrategy` 由 Contracts 拥有；当前 Catalog 直接拒绝 CandidateState/AlgebraicSolve。`ADR-0016` 处于 Proposed。该子切片没有完整 `PreparedModelPlan`/`QueryPlan`/`ClosurePlan`、`PreparedModelKey`、cache/lifetime、授权 caller、ABI/layout、linked entry、RuntimeInstance、IntegrationScope 或 Session，因此不改变 `R2-PLAN-001` 的 planned 状态。

## 后续阶段边界

1. `R2-CAT-001` 为 `review`；`R2-PLAN-001`、`R2-PRF-001`、`R2-LINK-001` 与 `R2-GATE-001` 保持 `planned`。Compiler 解析 RuntimeComponent definition 后会以 `GNC-PLAN-RUNTIME-COMPONENT-UNAVAILABLE` 拒绝发布局部 IR/plan。portable query/closure execution inputs 不能用于宣称这些任务完成或进入 PRF/LINK。
2. 当前最小阻碍是产品尚无 package-owned RigidBody ContinuousStateOwner/StateSchema、initial mapping 与 `PublishProjection -> CommittedRigidObservation` 合同；RigidStep 也未拆出正式 DerivativeEvaluation entry。产品 wrapper 内联生成 observation 并调用 guidance/controller/actuator/propulsion/rigid/mass；把其中单个 kernel 独立降级会留下 required input，把 wrapper 视为单一 owner会合并 rigid/mass owner 并提前改变 R3 state/time/transaction ownership。environment 每 tick 数据应来自 package PureQuery，而不是伪装成 RuntimeComponent 或 `RunBinding`。
3. StateOwner、DecisionAuthority、entity activation/topology、intervention/fault routing 和 RuntimeComponent sampled graph cycle analysis 尚未被闭合 plan 触发。既有 rigid/mass atomic boundary 继续归入未来 `IntegrationScopePlan` 与 `StepTransaction` 的多 owner 原子协调；这些公共合同需要仓库所有者选择先落在 R2/R3 哪个阶段后才能继续。
4. JSON/YAML/INI、多端 adapter、Session、mini runtime、manager、runtime registry、serializer、StateFragment、Artifact、Workflow、前端与 R3+ 能力继续保持关闭。
